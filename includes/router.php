...
class Router
{
    ...
    {         
        ...
        if(file_exists(RBNO_MODELS . DIRECTORY_SEPARATOR . strtolower($task) . '.php'))
        {
            require_once(RBNO_MODELS . DIRECTORY_SEPARATOR . strtolower($task) . '.php');

            $classname  = '\Rubinault\Framework\model' . ucfirst($task);          
    ...
    {
        $page   =   Factory::getPage();      
        $this->model = new \Rubinault\Framework\Model($this->params);
        $this->model->display();
        $page->setFile(null);
    }
