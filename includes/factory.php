...
ob_start();
require_once RBNO_ROOT . '/vendor/autoload.php';
require_once RBNO_CONFIGURATION . '/configuration.php';
ob_end_clean();

// Load the include needed classes
if ($folder = opendir(RBNO_INCLUDES)) {
    while (false !== ($file = readdir($folder))) {
        if ($file != "." && $file != ".." && $file != 'factory.php') {
            require_once RBNO_INCLUDES . DIRECTORY_SEPARATOR . $file;
        }
    }
    closedir($folder);
}
...
class Factory
{
    ...
    public static function getTheme()
    {
        if (!static::$theme) {
            $config = self::getConfig();
            static::$theme = RBNO_THEMES . DIRECTORY_SEPARATOR . $config->theme;
        }
        return static::$theme;
    }
    ...
    {
        if (file_exists(RBNO_MODELS . DIRECTORY_SEPARATOR . strtolower($name) . '.php')) {
            require_once(RBNO_MODELS . DIRECTORY_SEPARATOR . strtolower($name) . '.php');
    ...
    public static function jsBridge()
    {
        ...
        switch ($task) {
            ...
            case 'keepalive':
                $user = self::getUser();
                $source = ['success' => $user->isLogged(), 'message' => 'logged in'];
                break;
            case 'theme':
    ...
    public static function validToken()
    {
        ...
        //For non-logged users and a given token, check its validity
        $token  = $_GET['_t'];

        //Check the registry of exports. Deeper check at the link level
        $registry = RBNO_USERS . DIRECTORY_SEPARATOR . 'exports' . DIRECTORY_SEPARATOR . 'registry.xml';
            ...
            if ($node) {
                ...
                if ($attr['token'] == $token) {
                    ...
                    foreach ($links as $link)
                        if ($uri == $link->__toString()) $ret = true;
                }
            }
        }
        return $ret;
