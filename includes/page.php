...
class Page
{
    ...
    {
        self::sendMessage( $title , $content, true);
    }

    public static function sendSuccess( $title , $content)
    {
        self::sendMessage( $title , $content , false);
    }

    public static function sendMessage( $title , $content, $error = false)
    {
        $type = $error ? 'true':'false';
        $notify = "$.rbno.toast('<b>". addslashes($title) ."</b><br />" . addslashes($content) . "', $type );";
    ...
    public static function addStatic( $type = null)
    {
        ...
        $list = [];
        if ($handle = opendir(RBNO_STATIC))
        {
            while (false !== ($file = readdir($handle)))
            {
                if ($file != "." && $file != "..")
                {
                    $filepath   = RBNO_STATIC . DIRECTORY_SEPARATOR . $file;
